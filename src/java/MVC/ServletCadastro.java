...
public class ServletCadastro extends HttpServlet {
    ...
    protected void processRequest(HttpServletRequest request, HttpServletResponse response)
        ...
        try (PrintWriter out = response.getWriter()) {
            try{
                UserModel f = new UserModel(request.getParameter("name"),request.getParameter("user"),request.getParameter("password"));
                userDAO u = new userDAO();
                if(u.newUser(f)){
                	u.insere(f);
                	ServletContext app = this.getServletContext(); 
                    RequestDispatcher rd = app.getRequestDispatcher("/cadastrocorreto.jsp"); 
                    rd.forward(request, response);
                }
                else {
                	ServletContext app = this.getServletContext(); 
                    RequestDispatcher rd = app.getRequestDispatcher("/usuarioinvalido.jsp"); 
                    rd.forward(request, response);
                } 
            }
